...
import com.mymonthlyexpenses.management_system.model.ShoppingItem;
import com.mymonthlyexpenses.management_system.model.ShoppingItemCategory;
import com.mymonthlyexpenses.management_system.model.ShoppingItemUnit;
import com.mymonthlyexpenses.management_system.model.Store;
import com.mymonthlyexpenses.management_system.model.StoreItem;
import com.mymonthlyexpenses.management_system.view.AddFilterDialogFragment;
import com.mymonthlyexpenses.management_system.view.CategoriesSpinnerOnItemSelectedListener;
import com.mymonthlyexpenses.management_system.view.StorePickerDialogFragment;
import com.mymonthlyexpenses.management_system.view.StoresSpinnerOnItemSelectedListener;
import com.mymonthlyexpenses.management_system.view.StorePickerDialogFragment.StorePickerDialogListener;
import com.mymonthlyexpenses.management_system.view.UpdateStoreItemDialogFragment.UpdateStoreItemDialogListener;
...
public class MainActivity extends FragmentActivity implements
	...
	/*
	 * In order to make sure we are updating the correct store We are going
	 * to show the user a dialog with radio buttons for each store and have
	 * them choose the store they are going to work on
	 */
	...
	alertDialogBuilder.setPositiveButton("Yes",
		new DialogInterface.OnClickListener() {
			...
			task.execute(fileLocation,
				"http://192.168.1.105/management/syncToServer.php");
		    }
    ...
    /*
     * This method will initialize all of our helper arrays based on our local
     * json files
     */
    ...
    /*
     * This is a method that is called once we come back from our item update
     * dialog. Everything that has to do with updating items in our
     * ArrayAdapters, JSON files and ArryList's Is done here.
     */
	...
	/*
	 * Sometimes, I dont know exactly how and when, both the category and
	 * store id are not being put into the json array.
	 */
	...
	try {
		...
		/*
		 * In an atempt to solve the issue where we have corupted json
		 * object, instead of using the storeId and
		 * shoppingItemCategoryId variables, I am going to use the same
		 * information from the updatedItem object.
		 */
		storeItemJSONObject
    ...
    /*
     * This method will sync our application from the database in a dedicated
     * thread and onces its finished update our UI thread.
     */
	...
	Thread syncFromServerThread = new Thread() {
		...
		try {
		    readAndSaveJSONFeed(
			    "shopping_items.json",
			    "http://192.168.1.105/management/managementController.php?sync_shopping_items=from_server");

		    readAndSaveJSONFeed(
			    "shopping_item_category.json",
			    "http://192.168.1.105/management/managementController.php?sync_shopping_item_category=from_server");

		    readAndSaveJSONFeed(
			    "stores.json",
			    "http://192.168.1.105/management/managementController.php?sync_stores=from_server");

		    readAndSaveJSONFeed(
			    "store_items.json",
			    "http://192.168.1.105/management/managementController.php?sync_store_items=from_server");

		    readAndSaveJSONFeed(
			    "shopping_items_unit.json",
			    "http://192.168.1.105/management/managementController.php?sync_shopping_item_unit=from_server");
    ...
    /*
     * This method is called once we pick our filter date. This is where the
     * actual filtering of the ArrayAdapters takes place.
     */
    ...
    /*
     * This is the method that runs after we pick a store. This is where we
     * initialize all of our spinners and array adapters
     * 
     * @see com.mymonthlyexpenses.management_system.StorePickerDialogFragment.
     * StorePickerDialogListener#onFinishPickerDialogDialog(java.lang.String)
     */
